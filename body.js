...
<nav>
 ...
 /*this.drawImagesBlocks = function (id,ImagesBlock) {
    ...
    for (const [idx1, value] of ImagesBlock.entries()) {
      if (value.bigImage)
        imagesBlocks += `
        <li class="brands" onclick="customHomepage.setUUIDandNav(null,null,'${value.text}', 'customHomepage')"
  ...
  this.drawImagesBlocks = function (id, BrandsBlock) {
    let Brandshtml = "";
    for (const [idx1, value] of BrandsBlock.entries()) {
      Brandshtml += `
      <style>
      .${value.text} .sub${value.subtext} {
        display:none;
      ...
      .${value.text}:hover .sub${value.subtext}{
        display:block;
        padding-top: 20px;
        margin-right: 120px;
      }
      </style>
    <li id="brands" class="${value.text}"><a>${value.text}</a>
  <ul id="subbrands" class="sub${value.subtext}" onclick="customHomepage.setUUIDandNav(null,null,'${value.subtext}', 'customHomepage')"><a>${value.subtext}</a>
  </ul></li>`;
    }
    if(document.getElementById(id))
    document.getElementById(id).innerHTML = Brandshtml;
  };
...
this.getAccountInternalID = function () {
    ...
    var bridgeObject = {     
        fields: ["Name", "InternalID", "UUID", "TSAFRANCO"], //"TSACreditLine", "TSABalance"
    //   filter:{
    //       ApiName:"UUID",
    //       Operation:"IsEqual",
